    ...
    <div class="w-full max-w-md">
      <div class="rounded-2xl border border-slate-200 bg-white p-8 shadow-lg dark:border-slate-700 dark:bg-slate-800">
        ...
        <div class="mb-8 text-center">
          ...
          <p class="mt-2 text-sm text-slate-500 dark:text-slate-400">Entre na sua conta</p>
        </div>
        <form @submit.prevent="submit" class="space-y-5">
          ...
          <div class="flex items-center justify-between">
            ...
            <Link v-if="route().has('password.request')" :href="route('password.request')" class="text-sm text-emerald-600 hover:text-emerald-700 dark:text-emerald-400">
              Esqueceu a senha?
            </Link>
          </div>
          <button
        ...
        <p class="mt-6 text-center text-sm text-slate-500 dark:text-slate-400">
          Não tem conta?
          <Link :href="route('register')" class="font-medium text-emerald-600 hover:text-emerald-700 dark:text-emerald-400">Cadastre-se</Link>
        </p>
      </div>
    </div>
  </div>
